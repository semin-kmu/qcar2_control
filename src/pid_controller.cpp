...
private:
  ...
  void update()
  {
    ...
    if (gain_schedule_enabled_) {
      ...
      gain_scale = clamp(gain_scale, 0.1, 1.1);  // Prevent extreme gain scaling.
    }
    ...
    if (dt_pid > kEpsilon) {
      ...
      integrator_ = clamp(integrator_, i_min_, i_max_);
    ...
    double steer_cmd = clamp(steer_raw, -kMaxSteerRad, kMaxSteerRad);
  ...
  void publishSmoothedCmd(double dt, double target_speed, double target_steer)
  {
    ...
    // 3) Clamp and publish.
    cmd_msg_.values[0] = clamp(smoothed_steer_, -kMaxSteerRad, kMaxSteerRad);
    cmd_msg_.values[1] = clamp(smoothed_speed_, -1.0, 1.0);
