...
namespace hinf = qcar2_control::hinf;

static constexpr double kEpsilon = 1e-6;
static constexpr double kMaxDt = 1.0;
static constexpr double kMaxSteerRad = 0.6;

class HInfLateralControllerPHNode : public rclcpp::Node
{
public:
  HInfLateralControllerPHNode()
  : Node("h_inf_controller_ph_node")
  {
    ...
    rho_use_lookahead_curvature_ = this->declare_parameter<bool>("rho_use_lookahead_curvature", false);

    sub_guidance_ = this->create_subscription<qcar2_msgs::msg::LateralGuidance>(
      "/vfg/lateral_guidance", rclcpp::QoS(10),
      std::bind(&HInfLateralControllerPHNode::guidanceCallback, this, std::placeholders::_1));

    sub_path_ = this->create_subscription<qcar2_msgs::msg::PhQuinticPath>(
      "/planning/local_path_ph", rclcpp::QoS(1),
      std::bind(&HInfLateralControllerPHNode::pathCallback, this, std::placeholders::_1));
    ...
    timer_ = this->create_wall_timer(
      ...
      std::bind(&HInfLateralControllerPHNode::update, this));

    cmd_msg_.motor_names = {"steering_angle", "motor_throttle"};
    cmd_msg_.values.resize(2);

    std::string init_error;
    const double dt_model = 1.0 / std::max(1e-3, control_rate_hz_);
    hinf_ready_ = hinf_runtime_.initialize(dt_model, hinf::getLpvHinfV3Data(), init_error);
    if (!hinf_ready_) {
      RCLCPP_ERROR(this->get_logger(), "Failed to initialize LPV H-inf runtime: %s", init_error.c_str());
    }

    RCLCPP_INFO(this->get_logger(), "H-inf PH lateral controller started (%.1f Hz)", control_rate_hz_);
  }
...
private:
  ...
  void update()
  {
      ...
      latest_guidance_.curve_id == stop_curve_id_)
    {
      ...
      if (latest_guidance_.u_star >= stop_u_ && dist <= stop_distance_m_) {
        ...
        resetControllerState();
        publishStopCmd();
        RCLCPP_INFO(this->get_logger(), "Vehicle stopped at target location (PH, H-inf)");
        return;
      }
    }
      ...
      (latest_guidance_.header.stamp.nanosec != 0))
    {
      ...
      if (have_prev_guidance_stamp_ && stamp > prev_guidance_stamp_) {
        (void)(stamp - prev_guidance_stamp_).seconds();
      }
    ...
    double rho_curvature = latest_guidance_.curvature_1pm;
    if (rho_use_lookahead_curvature_ && std::isfinite(latest_guidance_.lookahead_max_curvature_1pm)) {
      rho_curvature = latest_guidance_.lookahead_max_curvature_1pm;
    }
    if (!std::isfinite(rho_curvature)) {
      rho_curvature = 0.0;
    }

    const double rho = std::fabs(rho_curvature) * std::fabs(target_speed);
    const double steer_raw = hinf_runtime_.compute(e, last_steer_cmd_, rho);
  ...
  bool rho_use_lookahead_curvature_{false};
  ...
  hinf::LPVHinfRuntime hinf_runtime_;
  bool hinf_ready_{false};
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<HInfLateralControllerPHNode>());
  rclcpp::shutdown();
  return 0;
}
